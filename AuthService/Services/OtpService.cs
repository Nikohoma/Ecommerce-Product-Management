...
namespace Auth.Services
{
    public class OtpService
    {
        ...
        public async Task SendOtpAsync(string email, string purpose)
        {
            ...
            {
                Console.WriteLine($"Invalid input: {ex.Message}", ex); return ;
            }
            catch (DbUpdateException ex)
            {
                Console.WriteLine("Database error while generating OTP.", ex); return;
            }
            catch (Exception ex)
            {
                Console.WriteLine("Failed to send OTP.", ex); return;
            }
        }
