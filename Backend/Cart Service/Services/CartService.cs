    ...
    public class cartService : ICartService  
    {
        ...
        public async Task<CartResponseDto> GetCartAsync(string userId)
        {
            ...
            foreach (var item in cart.Items)
            {
                try
                {
                    var product = await FetchProductAsync(item.ProductId);
                    response.Items.Add(new CartItemResponseDto
                    {
                        ...
                        ImageUrl = product.Media?.FirstOrDefault()?.MediaUrl ?? ""
                    });
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Failed to enrich cart item {ProductId} for user {UserId}", item.ProductId, userId);
                    // Add basic info if enrichment fails
                    response.Items.Add(new CartItemResponseDto
                    {
                        Id = item.Id,
                        ProductId = item.ProductId,
