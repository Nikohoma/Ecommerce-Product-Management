...
namespace ECommerceProductManagement.Services
{
    public class PasswordHasher
    {
        ...
        public string Hash(string password)
        {
            ...
            {
                throw new Exception($"Invalid password: {ex.Message}", ex);
            }
            catch (CryptographicException ex)
            {
                throw new Exception("Error occurred during password hashing.", ex);
            }
            catch (Exception ex)
            {
                throw new Exception("Unexpected error while hashing password.", ex);
            }
        }
